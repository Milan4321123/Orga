...
(function () {
  ...
  var SCHEMA = {
    events: {
      labelDe: "Veranstaltungen", labelEn: "Events",
      ...
      blank: function () { return { id: "ev-" + Date.now(), date: "", time: "", de: "", en: "", placeDe: "", placeEn: "", descDe: "", descEn: "", tags: "" }; },
      fields: [
    ...
    news: {
      ...
      titleKey: "de",
      sort: function (a, b) { return (a.date || "") > (b.date || "") ? -1 : 1; },
      blank: function () { return { id: "n-" + Date.now(), date: new Date().toISOString().slice(0, 10), tag: "verein", de: "", en: "", exDe: "", exEn: "", bodyDe: "", bodyEn: "" }; },
      fields: [
        { key: "date", de: "Datum", en: "Date", type: "date", required: true, half: true },
  ...
  var FIGURES = [
    { key: "schoolsReached", de: "Erreichte Partnerschulen", en: "Partner schools reached" },
    { key: "schoolsTarget", de: "Ziel: Schulen bis 2030", en: "Target: schools by 2030" },
    { key: "studentsTrained", de: "Geschulte Schüler:innen", en: "Students trained" },
    { key: "teachersCertified", de: "Zertifizierte Lehrkräfte", en: "Certified teachers" },
    { key: "volunteers", de: "Registrierte Volunteers", en: "Registered volunteers" },
    { key: "countries", de: "Länder der Diaspora", en: "Diaspora countries" },
    { key: "donationsEur", de: "Spenden bisher (€)", en: "Donations so far (€)" },
    { key: "donationGoalEur", de: "Jahresziel Spenden (€)", en: "Annual donation goal (€)" }
  ];

  var ORG_FIELDS = [
    { key: "email", de: "E-Mail allgemein", en: "General e-mail" },
    { key: "emailMembership", de: "E-Mail Mitgliedschaft", en: "Membership e-mail" },
    { key: "emailVolunteer", de: "E-Mail Volunteering", en: "Volunteering e-mail" },
    { key: "emailDonation", de: "E-Mail Spenden", en: "Donations e-mail" },
    { key: "phone", de: "Telefon", en: "Phone" },
    { key: "address.street", de: "Straße und Hausnummer", en: "Street and number" },
    { key: "address.zip", de: "PLZ", en: "ZIP code" },
    { key: "address.city", de: "Ort", en: "City" },
    { key: "bank.holder", de: "Kontoinhaber", en: "Account holder" },
    { key: "bank.iban", de: "IBAN", en: "IBAN" },
    { key: "bank.bic", de: "BIC", en: "BIC" },
    { key: "bank.bank", de: "Bank", en: "Bank" }
  ];
  ...
  function fieldMarkup(def, value, path) {
    var label = T(def.de, def.en) + (def.required ? ' <span class="req">*</span>' : "");
    var hint = def.hintDe ? '<span class="hint">' + esc(T(def.hintDe, def.hintEn || def.hintDe)) + "</span>" : "";
    var ph = def.phDe ? ' placeholder="' + esc(T(def.phDe, def.phEn || def.phDe)) + '"' : "";
    var input;
  ...
  function renderCollection(name) {
    ...
    if (!items.length) {
      host.innerHTML = '<p class="text-muted">' + T("Noch keine Einträge.", "No entries yet.") + "</p>";
      return;
    }
    host.innerHTML = items.map(function (item, i) {
      var title = esc(item[schema.titleKey] || T("(ohne Titel)", "(untitled)"));
      return '<details class="acc-item ce-row" data-index="' + i + '">' +
        ...
        '<button class="btn btn-sm btn-ghost" type="button" data-dup data-coll="' + name + '" data-i="' + i + '">' + T("Duplizieren", "Duplicate") + "</button>" +
        '<button class="btn btn-sm btn-ghost" type="button" data-del data-coll="' + name + '" data-i="' + i + '" style="color:var(--err)">' + T("Löschen", "Delete") + "</button>" +
        "</div></div></details>";
    }).join("");
  }
  ...
  function renderFigures() {
    ...
    host.innerHTML = '<div class="form-grid">' + FIGURES.map(function (f) {
      return '<div class="field"><label>' + T(f.de, f.en) + "</label>" +
  ...
  function renderAll() {
    ...
    if (stamp) {
      stamp.textContent = state.data.updatedAt
        ? T("Zuletzt gespeichert: ", "Last saved: ") + new Date(state.data.updatedAt).toLocaleString(T("de-DE", "en-GB")) +
  ...
  function save() {
    ...
    btn.disabled = true;
    btn.innerHTML = T("Wird gespeichert …", "Saving …");
    ...
    fetch(API + "/admin/content", {
      ...
      if (!res.ok) {
        ...
        window.npjoeToast(
          T("Nicht gespeichert. Bitte prüfen: ", "Not saved. Please check: ") + fields.join(", "),
          "err"
        );
        return;
      }
      ...
      window.npjoeToast(T("Gespeichert. Die Website zeigt die Änderung sofort.",
                          "Saved. The website shows the change immediately."), "ok");
    }).catch(function () {
      btn.disabled = false;
      btn.innerHTML = original;
      window.npjoeToast(T("Speichern fehlgeschlagen — läuft der Server?", "Saving failed — is the server running?"), "err");
    });
  }
  ...
  function bind() {
    ...
    root.addEventListener("click", function (e) {
      ...
      if (b.hasAttribute("data-del")) {
        if (!confirm(T("Diesen Eintrag wirklich löschen?", "Really delete this entry?"))) return;
      ...
      if (b.id === "contentReload") {
        if (state.dirty && !confirm(T("Nicht gespeicherte Änderungen verwerfen?", "Discard unsaved changes?"))) return;
        load().then(function () { window.npjoeToast(T("Neu geladen.", "Reloaded."), "ok"); });
      }
    });
